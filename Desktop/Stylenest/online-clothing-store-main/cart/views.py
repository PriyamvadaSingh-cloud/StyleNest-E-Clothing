...
class ProductList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tags"] = tags
        context["title"] = "StyleNest | All products"
        return context
...
class CategoryProductList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = f" StyleNest | {category} products"
        return context
...
class TagProductList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = f" StyleNest | {tag} products"
        return context
...
class CartView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "StyleNest | Cart items"
...
class UpdateCartItem(UserPassesTestMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = f"StyleNest | {product.title}"
...
class Checkout(RedirectZeroItem, FormView):
    ...
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs["user_id"] = self.request.user.pk if self.request.user.is_authenticated else None
        return kwargs
    ...
    def form_valid(self, form):
        ...
        if selected_shipping_address:
            ...
            # For guest checkout, create or fetch a user by the provided email
            if self.request.user.is_authenticated:
                ...
                if email:
                    ...
                    if not user:
                        # create a user with a random password; they remain inactive until they reset/activate
                        random_pw = create_ref_code()
                        user = User.objects.create_user(email=email, password=random_pw)
                        user.is_active = False
                        user.save()
                else:
                    user = None

            # Address model requires a user; attach to created/found user when possible
            if user:
                ...
                # attach order to this user so payment can use order.user.email
                order.user = user
            else:
                # fallback: do not create address if no user/email supplied
                pass
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = "StyleNest | Checkout"
        return context
